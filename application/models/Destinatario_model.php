...
class Destinatario_model extends CI_Model {
  ...
  /**
    ...
    function get($perpage=0,$start=0,$one=false){
        ...
        $this->db->select('destinatario.*, cargo.nomeCargo as cargo');
        $this->db->order_by('idDestinatario','desc');
        $this->db->limit($perpage,$start);  
        $this->db->join('cargo', 'destinatario.cargoId = cargo.idCargo', 'left');        
    ...
    function getById($id){
        ...
        $this->db->limit(1);
        return $this->db->get('destinatario')->row();
    }
    ...
    function search( $pesquisa, $de, $ate, $perpage=0,$start=0,$one=false){
        ...
        if($pesquisa != null){
            $this->db->like('nomeDestinatario',$pesquisa);
